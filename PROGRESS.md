...
### AI-Time Meter — billable time auto-capture (2026-05-19)
- **Header `SessionTimerPill`** in chat TopNav: hidden until first message, then live HH:MM:SS with pulsing status dot. Click → menu: Pause / End session & log time / Discard. 2-min idle pause is silent (no popup).
- **`BillingDraftModal`** opens on `handleNewThread` / sign-out / `yourai:end-session` event. Shows raw active time + rounded billable band, matter (required), client, **billable / non-billable toggle**, activity dropdown, AI-generated description, internal notes. Two save paths: Save as draft / Save & approve.
- **AI description** via `/api/chat` with a tight system prompt (one sentence, 15-25 words, past tense, strong verb, no "AI" mention, no first person). Streams the response, defensive quote-strip. Falls back to a deterministic `"Reviewed and analyzed material relating to <first user message>"` stub if `/api/chat` errors (e.g. dev environment without `OPENAI_API_KEY`).
- **`MyTimePanel`** — attorney's own log, sibling panel under chat sidebar "My Time" item. Same **table shell** as TeamTimePanel minus the Attorney column (Date · Matter · Activity · Description · Billable · Status · Actions). Tabs All / Drafts / Approved (no Exported — too confusing), search, in-row edit via `colspan=7` expanded form, CSV export. KPIs: Entries · Billable · Non-billable (when > 0) · Approved-in-filter.
- **`TeamTimePanel`** — Org Admin sibling panel under "Team Time" sidebar item (visible to `isOrgAdmin || hasPermission(ACCESS_BILLING)`). Mounted in ChatView, not at `/app/*` — attorneys live in chat, not in the org-admin portal. Five-card KPI strip, attorney + date-range filters, same table.
- **`OrgSettings → Time & Billing` card** (General tab): E-Billing Mode toggle (10 firm categories ↔ 25 UTBMS task+activity pairings) + Rate Increment (0.1 hr / 0.25 hr). Auto-saves on change.
- **Module-level singleton timer** (`src/lib/sessionTimer.ts`) mirrors the `workflowRunner` pub-sub pattern — must outlive component re-renders + sibling-panel swaps. Idle freeze pins accumulated seconds to `lastActivityAt`, not `now` — we never bill the 2-min gap. **One timer at a time** (CosmoLex model): switching threads finalizes the previous session.
- **Status machine is just `draft` → `approved`** (dropped `exported`; the CSV export is a one-click action, doesn't need its own status — PM said it was confusing).
- **No matter / file number** field — solo and small firms use the matter name as the identifier; e-billing firms get a separate file number via the UTBMS code anyway (PM call mid-session).
- **Localstorage stores**: events `yourai_billing_events_v1`, settings `yourai_billing_settings_v1`. Mock-API shape so the future REST endpoints drop in.
- **22-event demo seed** across 4 mock attorneys (Ryan Melade, Sarah Chen, James Wu, Maria Torres) via `src/data/demoBillingEvents.ts` — 4 drafts, 18 approved, 3 non-billable; ~22.6 h billable + 2.7 h non-billable. Dates are relative-to-today so date-range filters always populate. Seeded idempotently from ChatView's first-mount `useState`. Client demo lands on a populated view with zero setup.
- **Research + client-facing proposal**: [`docs/extracted/AI_Time_Meter_Research.md`](docs/extracted/AI_Time_Meter_Research.md) (dev-side reference) + [`docs/extracted/AI_Time_Meter_Proposal.md`](docs/extracted/AI_Time_Meter_Proposal.md) + [`docs/extracted/AI_Time_Meter_Proposal.docx`](docs/extracted/AI_Time_Meter_Proposal.docx) (client-facing, 10 embedded screenshots — 6 ours + 4 competitor marketing pages). Build script: `build_ai_time_meter_proposal_docx.cjs`.
- **Workspaces integration explicitly out of scope** for v1 — meter runs only in `/chat`, not `/chat/workspaces/:id`.
- **Open: hourly rates** not captured per event in v1; firm computes externally. A per-attorney `RateCard` model is the obvious next step before any revenue-forecast view.
- **Open: LEDES 1998B export** — only matters for insurance-defense / corporate-counsel firms. CSV-only in v1; `billingExport.ts` structured to add LEDES when first target firm asks.
- Pattern reference: `.claude-context/ai-time-meter.md`.
...
0. **AI-time meter — settle the four PM-level open questions** before any client demo using the feature. (a) **Hourly rates**: capture on the event, on the operator profile, or both? Required before any revenue-forecast view. Simplest answer: per-attorney `RateCard` on the operator with override-per-event for special-rate matters; this matches how Clio and CosmoLex model it. (b) **LEDES 1998B export priority**: only matters if we have an insurance-defense / corporate-counsel firm asking. If not, defer indefinitely — CSV covers 95% of small / solo firms. (c) **Mid-session matter switching**: wait for real attorney feedback (Wendy + Ryan Hoke if they kick the tires), or pre-empt with a Split-this-Session affordance in the draft modal? Pre-empting is ~half a day; waiting is free. (d) **Cross-device timer state**: depends on backend roadmap. Once the dev team's backend exists, write the timer ticks to a `sessions` table.
0`-ii`. **Demo the AI-time meter to Wendy + Ryan Hoke** in the next client touchpoint. Talking points: header timer pill (just works, no setup), draft modal with AI auto-summary (this is the wedge — Clio Manage AI is a $40/mo upgrade, we ship it native), one-timer-at-a-time (vs Bill4Time's concurrent), non-billable toggle for casual use, Team Time rollup for firm partners. Surface the four open questions above to get their input.
0`-iii`. **Hand the three Desktop deliverables to the dev team** (Bug Report DOCX, Bot Persona Prompts DOCX, Test Suite MD). Confirm with their lead which P0s they'll take first; the grounding-gate refusal is the single biggest unlock — fixing it lights up 6 of 11 enabled intents. Ask them to also share their /api/v1 schema once they update the intents table so we can stop pasting field values and start sending JSON.
...
**Five surfaces all wired to localStorage** (`yourai_billing_events_v1` for events, `yourai_billing_settings_v1` for org config — shaped exactly like the eventual REST endpoints): `SessionTimerPill` (TopNav right), `BillingDraftModal` (opens on `handleNewThread` / sign-out / `yourai:end-session` event), `MyTimePanel` (sibling under "My Time"), `TeamTimePanel` (sibling under "Team Time", visible to `isOrgAdmin || hasPermission(ACCESS_BILLING)`), `OrgSettings → Time & Billing` card. All five surfaces use the same `loadEvents()` / `updateEvent()` / `deleteEvent()` API.

**Three iterations on the way to v1**, each PM-driven, all the same day:

- **v1.1 — dropped `matterNumber`.** Field was clutter for solo and small-firm attorneys who use the matter name as the identifier; insurance-defense / corporate-counsel firms get a separate file number via the UTBMS code anyway. Removed from model + modal + table + CSV.
- **v1.2 — dropped `exported` status; moved Team view into ChatView.** PM: "The status is confusing, just keep it draft or something, exported should not be a status." CSV export is a one-click action, doesn't need its own state. Status machine is now `draft` → `approved`. Also: `/app/time-billing` route deleted, `BillingTimePage.jsx` renamed to `src/components/chat/TeamTimePanel.jsx`, mounted as a sibling panel inside ChatView with a "Team Time" sidebar item. PM: "This should be visible in chatview not on app view for org admin" — attorneys live in chat, not in the org-admin portal, so the Team view goes where they already are.
- **v1.3 — My Time uses the same table as Team Time** (minus the Attorney column). Edit-in-place uses a `colspan=7` expanded row instead of swapping the whole card out. KPIs unchanged.

**Architectural decisions worth remembering** (full notes in `.claude-context/ai-time-meter.md`):

- **`src/lib/sessionTimer.ts` is a module-level singleton** with a pub-sub `subscribeTimer()` API (mirrors `workflowRunner`). The timer must outlive component re-renders and sibling-panel swaps. Idle pause freezes `accumulatedSeconds` at `lastActivityAt`, not `now` — we don't bill the 2-min gap.
- **Switching threads without finalizing drops the prior session** (defensive — better than billing the wrong matter). `handleNewThread` calls `finalizeTimer()` first.
- **AI description calls `/api/chat`** with `{ message, history, system }`; user content is a transcript of up to 30 messages truncated to 600 chars each. Deterministic fallback when `/api/chat` errors: `"Reviewed and analyzed material relating to <first user message>"` + "Auto-summary unavailable" hint.
- **ChatView messages are `{ sender: 'user' | 'bot', content }`** — not `{ role, content }`. Normalized at the modal boundary. This bit me first time: modal showed "0 messages" + ignored the transcript. Now CLAUDE.md gotcha #39.
- **`useAuth().operator` returns null in dev** (no real session cookie); panel boundaries must fall back to `{ id: currentUserId, name: ORG_CURRENT_USER.name }` or saved events get `attorneyId: 'unknown'` and the My Time filter (`attorneyId === operator?.id`) filters everything out. Now CLAUDE.md gotcha #40.

**Demo seed data.** [`src/data/demoBillingEvents.ts`](src/data/demoBillingEvents.ts) builds **22 realistic events** across **4 attorneys** (Ryan Melade, Sarah Chen, James Wu, Maria Torres) — 4 drafts (today), 18 approved (1-19 days ago), 3 non-billable. **22.6 h billable + 2.7 h non-billable** total. Matters: Acme MSA Renewal, TechStart Due Diligence, Bradley v. Patel discovery, Chen Family Trust, Robertson Healthcare HIPAA audit, Acme Vendor MA, Employment Contract — Patel, plus Pro Bono Anchorage Tenant Coalition + Internal YourAI Training (non-billable). Dates relative-to-today so the date-range filters always populate. Seeded idempotently from ChatView's first-mount `useState` initializer alongside the KP / Vault seeds. **Client demo lands on a populated view with zero setup.**

**Files added (9):** `src/lib/aiTimeStore.ts` · `src/lib/sessionTimer.ts` · `src/lib/billingExport.ts` · `src/data/demoBillingEvents.ts` · `src/components/chat/SessionTimerPill.jsx` · `src/components/chat/BillingDraftModal.jsx` · `src/components/chat/MyTimePanel.jsx` · `src/components/chat/TeamTimePanel.jsx` · `build_ai_time_meter_proposal_docx.cjs`. **Files edited (4):** `ChatView.jsx` (imports, Sidebar props + items, TopNav slot, sendMessage start hook, handleNewThread finalize hook, `yourai:end-session` listener, panel + modal mounts, demo seed) · `App.jsx` (no new routes in v1.2 final state) · `OrgSidebar.jsx` (no entry — Team view lives in chat) · `OrgSettingsPage.jsx` (Time & Billing card on General tab). **Docs:** `docs/extracted/AI_Time_Meter_Research.md` (dev reference) + `docs/extracted/AI_Time_Meter_Proposal.md` + `.docx` (client-facing) + `.claude-context/ai-time-meter.md` (architecture reference).

**Open questions for PM** (in the proposal): hourly rates capture (event-level vs operator-level) · LEDES 1998B export priority (only matters if we have an insurance-defense firm asking) · mid-session matter switching · cross-device timer persistence (backend-dependent).
