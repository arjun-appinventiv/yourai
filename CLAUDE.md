...
- **Three-scope Search Within dropdown** (File Search / YourVault / Workspaces) on the chat input. Tried 2026-04-30, retired same day. PM read: "📁 YourVault — so complicated". The "scope" abstraction is engineer-speak; users don't think in scopes when attaching a doc to a chat message. Workspaces specifically is a confidentiality footgun on `/chat` (matter privilege between cases) — it lives in the sidebar.
