...
- **"Search my docs" toggle on the empty-state input** — single boolean (`searchMyDocs`), default OFF. ON = `sendMessage` also runs YourVault retrieval (token overlap across name + description + fileName + content, top-5 / 6 K cap, inlined under `[Documents attached to this conversation]`). Replaced the three-scope "Search Within" dropdown after PM feedback that scope-as-a-noun was too abstract. Workspaces is NOT a chat-input control — it lives in the sidebar as navigation. Do NOT reintroduce a multi-option dropdown here; specific items reach the chat via the persistent drop zone or the Vault page's "Use" button.
