...
- **Search Within dropdown picks a *scope*, never a specific item** — the empty-state input dropdown has exactly three options: File Search (default — current chat attachments), YourVault (private corpus search), Workspaces (routes to `/chat/workspaces`). State is `searchScope: 'files' | 'vault' | 'workspaces'`. Do NOT add per-doc / per-workspace listings inside this dropdown — that's the scaling problem the redesign was built to escape (30+ matters / 100+ docs blow up submenus). Specific items reach the conversation via the persistent drop zone (inline pills) or via the Vault page's "Use" button (sets `activeVaultDocument`). KP dropdown stays separate — packs are bounded user-curated bundles and don't suffer the listing problem.
