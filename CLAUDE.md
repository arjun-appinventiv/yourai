...
- **`closeAllPanels()` helper for full-page panel mutual exclusion** — `ChatView.jsx` defines a single helper that zeroes every `show*Panel` flag (Team / Workspaces / Workflows / Prompt / Clients / KnowledgePacks / DocumentVault / editingWorkflow). Every Sidebar + HomeTileLauncher open-handler must call it before setting its target flag — otherwise two full-page panels render side-by-side as flex peers (real bug seen 2026-04-27).
- **Sidebar `active` state is dynamic, derived from `sidebarActiveKey`** — never hard-code `active: true` on a nav item. The active key is computed from `initialView` + the show*Panel flags inside `ChatView`'s main component; full-page panels take precedence over the underlying chat / home in the precedence chain. Sidebar item objects then `.map((it) => ({ ...it, active: it.id === activeKey }))`.
- **Vault search currently checks `name + description + fileName` only — NOT content.** `d.content` is bundled on the seed docs (`SAMPLE_VAULT_CONTENT`) but the substring filter doesn't include it. Adding it is a 1-line change in `filteredDocs`; trade-off is client-side substring across N×100K-char strings gets laggy at scale (50+ real docs). Defer until the first user complains or the backend lands.
