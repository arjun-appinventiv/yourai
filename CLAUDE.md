...
- **General Chat is the post-login landing surface for non-external users** — there is no `/chat/home` route; the prior tile-launcher home page was retired 2026-04-30. Login / signup-invited / onboarding-complete all navigate straight to `/chat`. Externals still land on `/chat/workspaces`. Do NOT add a Home sidebar entry, a `/chat/home` route, or an `initialView="home"` branch back — the chat empty state's intent-pill row + Search Within dropdown + drop zone are the canonical entry points now.
- **"Search my docs" toggle + "From YourVault" picker on the empty-state input** — two YourVault affordances by mental model. Toggle (`searchMyDocs: boolean`, default OFF) = AI runs token-overlap retrieval at send time. Picker = user pins a specific vault doc via `activeVaultDocument`. Both reach the same corpus; the choice is AI-picks vs user-picks. Workspaces is NOT a chat-input control — it lives in the sidebar as navigation.
- **All pickers that list user-owned items use the search-first pattern** — sticky search input at the top of the popover (only renders when library > 5 items), filtered list below (substring match on name + description), footer link to the full management page. Applied to KP dropdown + From YourVault picker. Do NOT reintroduce flat-list popovers without a search input on libraries that can grow past ~10 items — the listing problem comes back. State: `kpQuery` / `vaultAttachQuery`, both clear on close.
- **Intent dropdowns render verb buckets, not flat lists** — `src/lib/intents.ts` exports `INTENT_BUCKETS` (DEFAULT / ASK & RESEARCH / ANALYZE / DRAFT) + `groupIntentsByBucket(ids[])` which filters empty buckets and preserves canonical order. Both the populated-chat collapsed-pill dropdown and the empty-state "More operations" overflow render from this. Bucket label = uppercase 10 px IBM Plex Mono with 0.12em tracking, divider line above non-first buckets. When adding a new intent: register it in `INTENTS`, then add its id to the appropriate bucket in `INTENT_BUCKETS` (don't leave it bucket-less — it'll silently disappear from both dropdowns).
- **Two distinct vault search paths — don't conflate them.** (1) The **YourVault page toolbar** search still checks `name + description + fileName` only (NOT content) — the substring filter in `filteredDocs` is one line away from including `d.content` but the trade-off is client-side substring across N×100K-char strings gets laggy at scale; defer until backend lands. (2) **YourVault scope retrieval in `ChatView.sendMessage`** — when `searchScope === 'vault'` and no explicit attachment, runs token-overlap relevance scoring across `name + description + fileName + content`, picks top-5 docs (cap 6 K chars per doc), inlines under the existing `[Documents attached to this conversation]` header. Stop-word filtered, word-boundary regex, score capped per token. **No backend dep** — works against the existing `documentVault` localStorage. Replace with pgvector retrieval later (`.claude-context/vault-content-rag-plan.md`) without UI churn.
- **Chat-attached files auto-save into YourVault — including extracted content.** `handleAttachFiles(kind='doc')` creates a vault entry on attach (metadata only, deduped by fileName via `addedFromChat: true`); the extraction `.then()` then backfills the entry's `content` field via the `vaultIdByFileName` map. Without this backfill, YourVault-scope retrieval would never find chat-attached docs. If a vault entry already has content (richer prior copy), skip the overwrite.
- **Drop zone is persistent across the conversation** — both prior gates (`showEmptyState` AND `pendingAttachments.length === 0`) are gone. The tile renders below the input throughout the chat, with reduced padding in populated state. Label flips to `Add another file (N attached)` when files exist; the chip row above the input handles per-file removal via X button.
...
20. **React Router routes that share a component with different props don't always remount** — when navigating from `/chat/home` (initialView='home') to `/chat/workspaces` (initialView='workspaces'), the `<ChatView>` instance can be reused. Any state initialised from `useState(initialView === 'workspaces')` will NOT re-init on the new prop. Fix: handlers that route between same-component routes must `setShowXxxPanel(true)` explicitly before navigating. The home tile's `onOpenWorkspaces` got bitten by this 2026-04-27 — *clicking the tile sent the URL to /chat/workspaces but the panel never opened* because the closeAllPanels-set false stuck.
...
25. **The home tile launcher renders inside `ChatView` when `initialView === 'home'`** — it's NOT a separate component file. The chat-main-area's `display: 'none'` condition handles whichever full-page panel is open; HomeTileLauncher takes the same slot when no panel is active. Login default for non-externals: `/chat/home`. Externals: `/chat/workspaces`.
...
- **`/chat/home` tile launcher** (six tiles: General Chat / Workspaces / YourVault / Workflows / Knowledge Packs / Invite Team). Shipped 2026-04-27, retired 2026-04-30. PM read: an extra click without surfacing anything the chat empty state doesn't already cover. The intent-pill row + Search Within dropdown + drop zone are the canonical entry points; do not add a Home sidebar entry, an `initialView="home"` branch, or a `/chat/home` route back.
- **Per-item submenus inside the chat Search Within / Source dropdown** (`Workspace ▸` listing each workspace, `YourVault ▸` listing each vault doc). Tried 2026-04-29; broke at scale (30+ matters / 100+ docs make the popover unscrollable). Replaced 2026-04-30 by the three-scope flat `Search Within` dropdown — it picks a *scope*, never an item. Specific items reach the chat via the persistent drop zone (inline pills). Don't reintroduce the submenu pattern.
